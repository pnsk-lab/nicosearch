...
const App: Component = () => {
  ...
  createEffect(() => {
    ...
    setResults(
      Object.values(DB).filter((video: video) => regex.test(video.title)),
    )
  })
  return (
    <div class="px-10 sm:px-20 mb-20">
      <h1 class="mt-20 w-full text-center text-3xl font-cutiveMono">
        NICOSEARCH
      </h1>
      <input
        onChange={(e) => setKeys(e.target.value.replace('　', ' ').split(' '))}
        placeholder="And検索はスペース区切りで！"
        class="mt-5 border border-gray-400 border-1 w-full h-14 px-5 rounded-full"
      />
      <Show when={keys().length && keys()[0] != ''}>
        <For each={results()}>{(result) => <Result video={result} />}</For>
      </Show>
